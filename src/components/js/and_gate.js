...
const AndGate = React.memo((props) => {
    ...
    useEffect(() => {
        ...
        if (props.inputs !== state.inputs) {
          ...
          if ((props.inputs.a && props.inputs.b) !== outputState) {
            setOutputState(props.inputs.a && props.inputs.b);
            onStateChange(id, props.inputs.a && props.inputs.b, 0);
          }
        }
    ...
    }, [props.selected, props.inputs, props.zoom, props.offset]);

    return (
            <g opacity={state.opacity} onMouseDown={dragStart} className={"Component-andgate-" + id} transform={"translate(" + state.position.x + "," + state.position.y + ") rotate(" + state.rotation + ")"} width="206" height="106" viewBox="0 0 206 106" fill="none">
                <g className="AndGate">
                    ...
                    <circle onMouseDown={(e) => {StartEndWire(e,id, "andgate", 1, "input")} } className="IO In-1" cx="8" cy="83" r="9" fill="#FF0000" stroke="black" strokeWidth="4"/>
                    <circle onMouseDown={(e) => {StartEndWire(e,id, "andgate", 0, "input")} } className="IO In-0" cx="8" cy="23" r="9" fill="#FF0000" stroke="black" strokeWidth="4"/>
                    <circle onMouseDown={(e) => {StartEndWire(e,id, "andgate", 0, "output")} } className="IO Out-0" cx="198" cy="53" r="9" fill="#FF0000" stroke="black" strokeWidth="4"/>
                </g>
            </g>
