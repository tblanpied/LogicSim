...
const AndGate = React.memo((props) => {
  ...
  const [state, setState] = useState({
    ...
    input_names:
      input_names !== undefined ? JSON.parse(input_names) : ["a", "b"],
    output_names:
      output_names !== undefined ? JSON.parse(output_names) : ["output"],
    zoom,
    offset,
    input_hovered: "",
  });
  ...
  useEffect(() => {
    ...
    if (
      input_names !== undefined &&
      input_names !== JSON.stringify(state.input_names)
    ) {
      setState((prevState) => ({
        ...prevState,
    ...
    if (
      ...
      output_names !== JSON.stringify(state.output_names)
    ) {
      setState((prevState) => ({
        ...prevState,
  ...
  return (
    ...
    >
      <g className="AndGate">
        <g className={`select-border ${state.selected ? "" : "display-none"}`}>
          <rect
        ...
        <g>
          <circle
            ...
            onMouseDown={(e) => {
              StartEndWire(e, id, "andgate", 1, "input");
            }}
            cx="8"
            cy="83"
            r="11"
          ...
          <g className={state.input_hovered === "In-1" ? "" : "display-none"}>
            <rect
              x={
                -(state.input_names[1].length
                  ? textRefs.current[1] !== undefined
                    ? textRefs.current[1].getComputedTextLength() + 10
                    : 0
                  : 0) - 15
              }
              y="72"
        ...
        <circle
          onMouseDown={(e) => {
            StartEndWire(e, id, "andgate", 0, "input");
          }}
          className="IO In-0"
          cx="8"
          cy="23"
        ...
        <g>
          ...
          <g className={state.input_hovered === "In-0" ? "" : "display-none"}>
            <rect
              x={
                -(state.input_names[0].length
                  ? textRefs.current[0] !== undefined
                    ? textRefs.current[0].getComputedTextLength() + 10
                    : 0
                  : 0) - 15
              }
              y="12"
        ...
        <g>
          <circle
            ...
            onMouseLeave={handleMouseLeave}
            onMouseDown={(e) => {
              StartEndWire(e, id, "andgate", 0, "output");
            }}
            cx="198"
            cy="53"
            r="11"
          ...
          <g className={state.input_hovered === "Out-0" ? "" : "display-none"}>
            <rect
              ...
              y="41"
              width={
                state.output_names[0].length
                  ? textRefs.current[2] !== undefined
                    ? textRefs.current[2].getComputedTextLength() + 6
                    : 0
                  : 0
              }
              height="23"
