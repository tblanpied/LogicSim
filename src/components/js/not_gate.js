...
const NotGate = React.memo((props) => {
  ...
  const [state, setState] = useState({
    ...
    input_hovered: "",
    input_names:
      input_names !== undefined ? JSON.parse(input_names) : ["input"],
    output_names:
  ...
  const _dragging = useCallback((e) => {
    ...
    if (
      ...
      (start_position.current.x !== e.pageX ||
        start_position.current.y !== e.pageY) &&
      (stateRef.current.selected || stateRef.current.new_component)
    ) {
      ...
      <g className="NotGate">
        ...
        <g>
          <circle
            ...
            onMouseDown={(e) => {
              StartEndWire(e, id, "notgate", 0, "output");
            }}
            cx="184.5"
            cy="76.5"
            r="11.5"
              ...
              width={
                state.output_names[0].length
                  ? textRefs.current[1] !== undefined
                    ? textRefs.current[1].getComputedTextLength() + 8
                    : 0
                  : 0
              }
              height="25"
        ...
        <circle
          onMouseDown={(e) => {
            StartEndWire(e, id, "notgate", 0, "input");
          }}
          className="IO In-0"
          cx="13.4999"
          cy="76.5"
