...
const PropertiesPanel = React.memo((props) => {
    ...
    useEffect(() => {
        ...
        if(component_name !== undefined && component_name !== state.component_name){
            setState((prevState) => ({
              ...prevState,
        ...
        if(properties !== undefined && properties !== JSON.stringify(state.properties)){
            setState((prevState) => ({
              ...prevState,
              properties: JSON.parse(properties)
            }));
    ...
    for(let i = 0; i < stateRef.current.properties.length; i++){
        properties_groups.push(
            <div key={`property_group_${state.component_id}_${i}`} className="property_group">
                <div key={`property_input_type_${state.component_id}_${i}`} className="property_input_type">
                    <div key={`property_name_${state.component_id}_${i}`} className="property_name">
                        {stateRef.current.properties[i].name}
                    </div>
                    <div key={`property_input_${state.component_id}_${i}`} className="property_input">
                        <input key={`property_input_text_${state.component_id}_${i}`} name={stateRef.current.properties[i].name} onChange={onChangeHandler} defaultValue={stateRef.current.properties[i].default} className="property_input_text" type="text"/>
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <div className={"properties_panel" + (state.display?"":" display-none")}>
